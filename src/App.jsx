...
export default function App() {
  ...
  // Microphone Setup with Audio Feedback Prevention (Mutes Mic when MYRAA Speaks)
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onresult = (event) => {
        // PREVENT AUDIO LOOP: Ignore mic input while MYRAA is speaking
        if (isSpeaking) {
          setLiveSpeechText('');
          return;
        }
        ...
        // Only capture user speech if not generated by AI speaking
        if (!isSpeaking && current.trim()) {
          setLiveSpeechText(current);

          if (silenceTimerRef.current) clearTimeout(silenceTimerRef.current);
          silenceTimerRef.current = setTimeout(() => {
            const finalSpeech = current.trim();
            if (finalSpeech && !isSpeaking) {
  ...
  // Pause Mic automatically whenever MYRAA starts speaking
  useEffect(() => {
    if (isSpeaking && recognitionRef.current && isListening) {
      try { recognitionRef.current.stop(); } catch (e) {}
    } else if (!isSpeaking && recognitionRef.current && isListening) {
  ...
  const speakText = (text) => {
    ...
    utterance.onstart = () => {
      setIsSpeaking(true);
      // Temporarily stop mic recognition while speaking
      try { recognitionRef.current?.stop(); } catch (e) {}
    };

    utterance.onend = () => {
      setIsSpeaking(false);
      // Resume mic recognition after speaking completes
      if (isListening) {
        setTimeout(() => {
          try { recognitionRef.current?.start(); } catch (e) {}
        }, 500);
  ...
  const handleSendMessage = async (customPrompt = null, screenshot = null) => {
    ...
    try {
      ...
      setMessages(prev => [
        ...
        {
          role: 'assistant',
          content: `Notice: ${err.message}. Please verify API settings!`,
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        }
      ]);
    } finally {
  ...
  const handleCaptureScreenVision = async () => {
    ...
    try {
      ...
      console.log('Backend screen capture fallback to WebRTC:', err);
    }
  ...
  return (
    <div className="relative w-screen h-screen max-h-screen bg-zinc-950 text-zinc-100 overflow-hidden font-sans select-none">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-zinc-500 font-mono tracking-wider flex items-center gap-1">
              ...
              {isProcessing ? 'THINKING...' : (isSpeaking ? 'TALKING...' : 'ONLINE & READY')}
            </p>
          </div>
        </div>
      ...
      {/* 3. Sideways Response Card (Right Side) */}
      ...
      {/* Live Speech Feedback (Only when user speaks) */}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 w-full max-w-xl px-4">
        <div className="bg-zinc-950/95 border border-zinc-800 focus-within:border-zinc-600 p-2 rounded-full backdrop-blur-2xl shadow-2xl flex items-center gap-2 transition">
          ...
          <input
            ...
            placeholder={isListening ? (isSpeaking ? "MYRAA is speaking..." : "Listening continuously...") : "Talk to MYRAA..."}
